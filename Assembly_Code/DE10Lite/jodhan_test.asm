...
send_temp_serial:
    ...
    ; tenths = (centi + 5) / 10   (round to nearest 0.1C)
    Load_y(5)
    lcall add32
    Load_y(10)
    lcall div32          ; x = tenths (we only trust quotient in x)
    ...
    ; Compute:
    ;   integer_degC = tenths / 10
    ;   frac_digit   = tenths % 10
    ; using repeated subtraction on 16-bit tenths
    mov R6, #0           ; Q low  (integer_degC)
    mov R5, #0           ; Q high
...
Div10_done:
    ; remainder (0..9) is in R4 (since R3 should be 0 now)
    ; quotient (integer_degC) is in R5:R6

    ; ---- print integer_degC as 3 digits (same style as your current code) ----
    mov R2, #0           ; hundreds
...
Hund_done:
    ...
    ; tens/ones from remaining (0..99) in R6 (R5 should be 0 here)
    mov a, R6
    mov b, #10
    div ab               ; A=tens, B=ones
    ...
    ; '.' then fractional digit
    mov a, #'.'
    lcall putchar
    mov a, R4            ; remainder 0..9
    add a, #'0'
    lcall putchar
