...
Set_Cursor(1,7)
	...
	; check tilt maybe only every 500ms to reduce SPI traffic:
    ; (easy version: check every loop; still ok)
...
ADXL345_Configure:
    ; DATA_FORMAT (0x31) = 0x08 (full resolution, +/-2g range bits=0)
    mov R0, #31h
    mov R1, #08h
...
Abs16_R3R2:
    mov a, R3        ; move high byte into A
    jb  acc.7, abs_neg   ; check sign bit (bit 7 of A)
    ret
...
TILT_TH_LO EQU 00h
TILT_TH_HI EQU 06h   ; 0x0600 (tune this)
...
Check_Tilt_Abort:
    ...
    ; Only enforce tilt abort when running (state != 0)
    mov a, FSM1_state
    jz  CTA_done
    ...
    ; if absX >= THRESH -> abort
    mov a, R3
    clr c
    subb a, #TILT_TH_HI
    jc  check_Y            ; absX < TH_hi
    jnz do_abort           ; absX > TH_hi
    mov a, R2
    clr c
    subb a, #TILT_TH_LO
    jnc do_abort           ; absX >= TH
...
do_abort:
    ; Hard abort: heater off + error state + alarm
    mov pwm, #0
    clr SSR_PIN

    mov FSM1_state, #6     ; your ERROR state
    mov tilt_fault, #1

    mov beep_count, #10    ; or make a louder alarm pattern
    clr TR0
    clr SOUND_OUT
