...
Copyright (C) 2005-2024  Jesus Calvino-Fraga / jesusc at ece.ubc.ca
...
extern volatile unsigned char xdata __at (0x80ff) rxcount;
extern volatile unsigned char xdata __at (0x8000) rxbuff[255];
extern volatile unsigned char xdata __at (0x8100-0x100+0x2fff) txcount;
extern volatile unsigned char xdata __at (0x8100) txbuff[0x2fff-0x100];

//These are the Special Function Register names for the MAX10_8052 microcontroller
...
char getchar(void)
{
	...
	if(P3_3==0) // Get data from serial port
	{
		while(RI==0); // Wait for data to arrive
		RI=0;
		c=SBUF;
		// Echo
		while (!TI);
	...
	else // Get data from shared debug memory.  The quartus_stp memory editor changes this memory.
	{
		if(rxbuff[0]==0) // If buffer empty, wait for data
		{
			j=rxcount; // Counter of the old message
			while(rxcount==j); // Wait for data to arrive. 'rxcount' is incremented using quartus_stp memory editor.
		}
		c=rxbuff[0];
...
void get_txbuff (void)
{
	...
	//	while(txcount!=0);
	//}
	//else // Using the serial port
	//{
		//txcount=pos;
		//for(pos=0; pos<txcount; pos++)
		//{
		//	while(!TI);
		//	TI=0;
		//	SBUF=txbuff[pos];
...
void add_nlcursor_txbuff (void)
{
	...
	txbuff[pos++]='>';
	txbuff[pos++]=' ';
	if((P3_3==0) && (pos>200)) flush_txbuff();
}
...
{
	if ((P3_3==1) && (append_txbuff==1)) return;
	...
	if(pos>0)
	{
		...
		{
			txcount++;
			// May have to wait half second or so
			_asm
				push AR2
				push AR1
				push AR0
...
void update_txbuff (void)
{
	...
	if(pos>8191) // Could it be bigger???
	{
