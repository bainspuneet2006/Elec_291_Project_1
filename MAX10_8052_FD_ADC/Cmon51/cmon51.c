...
volatile unsigned char xdata __at (0x80ff) rxcount;
volatile unsigned char xdata __at (0x8000) rxbuff[255];
volatile unsigned char xdata __at (0x8100-0x100+0x2fff) txcount;
volatile unsigned char xdata __at (0x8100) txbuff[0x2fff-0x100];
...
void put_txbuff(void)
{
	int j;
	for(j=0; txbuff[j]!=0; j++)
	{
		while (!TI);
		TI=0;
		SBUF=txbuff[j];
...
void clearline (void)
{
	...
	txbuff[j]='\r';
	txbuff[j+1]=0;
	txcount++; // Signal to the quartus_stp memory editor that new data is available
	if(P3_3==0) put_txbuff();
}
...
void getsn (void)
{
	...
	{
		j=rxcount;
		while(rxcount==j); // Wait for data to arrive
	}
	else
	{
		fill_rxbuff();
...
void modifymem(unsigned char * ptr,  char loc)
{
	...
	while(1)
	{
		...
		if(P3_3==0)
		{
			fill_rxbuff(); // Using the serial port
		}
		else
		{
			j=rxcount;
		...
		{
			for(j=0; (rxbuff[j]!=0) && (rxbuff[j+1]!=0) && (rxbuff[j+2]!=0) ; j+=3)
			{
