/*  hex2csv.c: Converts a hex file to csv to inport to excel
...
int set_options (char * opt)
{
	...
	switch (opt[0])
	{
		...
		case '?':
		case 'v':
		    printf("%s: HEX to CSV version %s\n", ProgName, version);
		    printf("by Jesus Calvino-Fraga, Copyright (c) 2021-2024\n\n");
...
int main(int argc, char **argv)
{
	...
	strcpy(CSVName, HexName);
	
	k=strlen(CSVName);
	for(j=k-1; j>0; j--) if(CSVName[j]=='.') break;
	if(j>0)
	{
		CSVName[j]=0;
		strcat(CSVName, ".csv");	
	...
	printf("Converting '%s' to '%s'...", HexName, CSVName);
	
    if ( (fileout=fopen(CSVName, "w")) == NULL )
    {
       printf("\nError: Can't create file `%s`.\n", CSVName);
       return -1;
    }
	
	MaxAddress=ReadHexFile(HexName);
	if(MaxAddress<0)
	{
		printf("\nERROR: Could not load file '%s'.", HexName);
		return -1;
	}
	
	for(j=0; j<MaxAddress; j++)
	{
		fprintf(fileout, "%d\n", FlashBuff[j]);
	}
